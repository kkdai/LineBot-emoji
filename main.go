...
func NewEmojiMsgWithEmoji(msg *linebot.TextMessage) linebot.SendingMessage {
	if len(msg.Emojis) > 0 {
		...
		prefixIndex := len("$ 你好 \n ")
		retObj := linebot.NewTextMessage(fmt.Sprintf("$ 你好 \n %s, 這是新的傳送 Emoji 的方式，如果你有 emoji 這裡會替換。", workMsg)).AddEmoji(linebot.NewEmoji(0, "5ac1bfd5040ab15980c9b435", "086")).AddEmoji(linebot.NewEmoji(prefixIndex+4, "5ac1bfd5040ab15980c9b435", "086"))
